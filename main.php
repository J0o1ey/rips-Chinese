...
<?php 
	...
	if(empty($_POST['search']))
	{
		...
		if($count_all > 0)
		{
			...
			echo '<tr><td nowrap width="160" onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="showAllCats()" style="cursor:pointer;" title="show all categories">共计:</td><td>',$count_all,'</td></tr>';
		} else
		{
			echo '<tr><td colspan="2" width="160">这程序太牛逼了，没找到漏洞.</td></tr>';
		}
	} else
	{
		echo '<tr><td colspan="2">',(($count_matches == 0) ? 'No' : $count_matches),' 符合条件的.</td></tr>';
	}

	echo '</table><hr /><table class="textcolor" width="100%">',
		'<tr><td nowrap width="160" onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="openWindow(5);eval(document.getElementById(\'filegraph_code\').innerHTML);maxWindow(5, 650);" style="cursor:pointer;" title="open files window">扫描的文件数:</td><td nowrap colspan="2">',count($files),'</td></tr>';
	if(empty($_POST['search']))
	{
		echo '<tr><td nowrap width="160">成功包含:</td><td nowrap colspan="2">';
		...
		echo '</td></tr>',
		'<tr><td nowrap>发现的链接:</td><td nowrap>',count($scan_functions),'</td><td rowspan="4" >';
		if(empty($_POST['search']) && $count_all > 0)
		{
			echo '<div class="diagram"><canvas id="diagram" width="80" height="70"></canvas></div>';
		}
		echo '</td></tr>',
		'<tr><td nowrap onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="openWindow(3);eval(document.getElementById(\'functiongraph_code\').innerHTML);maxWindow(3, 650);" style="cursor:pointer;" title="open functions window">用户使用的函数:</td><td nowrap>'.(count($user_functions_offset)-(count($user_functions_offset)>0?1:0)).'</td></tr>',
		'<tr><td nowrap onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';" onClick="openWindow(4);" style="cursor:pointer;" title="open userinput window">独特的来源:</td><td nowrap>'.count($user_input).'</td></tr>',
		'<tr><td nowrap>敏感的功能:</td><td nowrap>'.(is_array($file_sinks_count) ? array_sum($file_sinks_count) : 0).'</td></tr>',
		'</table><hr />';
		...
		if( !empty($info) || ($count_inc>0 && $round_inc_success < 75 && !$scan_subdirs && count($files)>1) )
		{
			...
			{
				echo '<tr><td width="160">Info:</td><td><small><font color="orange">你的包含成功率太低<i>勾选子目录</i>可以更好地扫描</font></small></td></tr>';
			}
			echo '</table><hr />';
		}
		
		echo '<center><a href="https://www.ripstech.com/latest/" target="_blank" style="text-decoration:none;font-size:11pt" onmouseover="this.style.color=\'white\';" onmouseout="this.style.color=\'#DFDFDF\';">来看看RIPS的下个版本吧！<font color="#FC4">RIPS</font><br />代码审计的艺术无处不在！</a></center><hr />';
	}	
		?>
		<table class="textcolor" width="100%">
		<tr><td nowrap width="160">扫描时间:</td><td nowrap><span id="scantime"><?php printf("%.03f seconds", $elapsed); ?></span></td></tr>
	</table>		
